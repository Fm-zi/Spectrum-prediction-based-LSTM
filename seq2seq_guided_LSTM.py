...
def main(x_l,y_l):
    ...
    x_length = x_l # the input sequence length
    y_length = y_l # the output sequence length
    percentage = 0.8 # the percentage of data used for training
    filename = "../Dataset/LTE.txt" #Set this to the dataset on which the model is to be run
    name_flag = "LTE.txt" # the name flag for the test case
    ...
    #print("-- generated the test cases --")
    #sys.exit(0)
    X_train_random = np.array(X_train_random_data)
    Y_train_random = np.array(Y_train_random_data)

    name = "Seq2Seq_"+name_flag+"_LSTM_Y_test_x_"+str(x_length)+"_y_"+str(y_length)+"data.txt"
    ...
    hidden_size = 50 # LSTM hidden node size
    ...
    rnn_model = graph(feed_previous=False) # for guided model
    saver = tf.train.Saver()
        ...
        while CONTINUE_FLAG:
            ...
            if ep % 100 == 0:
                ...
                if ep % 2000 == 0:
                    ...
                    name =  "Seq2seq_"+name_flag+"_LSTM_TRAIN_RMSE_x_"+str(x_length)+"_y_"+str(y_length)+"data.txt"
                    writetofile(name,train_RMSE) # saving the train RMSE values for every 200th epoch

                    name =  "Seq2seq_"+name_flag+"_LSTM_TRAIN_LOSS_x_"+str(x_length)+"_y_"+str(y_length)+"data.txt"
    ...
    rnn_model = graph(feed_previous=True) # the model will be take the previous output as the next input
    init = tf.global_variables_initializer()
    CONTINUE_FLAG = True # reset the continue flag

    with tf.Session() as sess:
        sess.run(init)
        saver = rnn_model['saver']().restore(sess,os.path.join(save_path_name,save_object_name))
        ...
        temp_y_found = temp_output.tolist()
        temp_err = RMSE(Y_train_random,temp_y_found)
        name = "Seq2seq_"+name_flag+"_LSTM_TRAIN_FOUND_RMSE_x_"+str(x_length)+"_y_"+str(y_length)+"data.txt"
    ...
    name = name_flag+"_"+str(x_length)+"_"+str(y_length)+"_pred.txt"
    writetofile(name,Y_found)

    err = RMSE(Y_test_data,Y_found)

    name = name_flag+"_"+str(x_length)+"_"+str(y_length)+"_test_rmse.txt"
